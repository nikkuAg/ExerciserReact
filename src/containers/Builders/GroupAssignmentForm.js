...
class GroupAssignmentForm extends Component {
  ...
  groupAlreadyUsed = () => {
    ...
    questions.forEach((question) => {
      if (
        parseInt(question.correctGroup.value.split("-").pop(), 10) ===
        groups.length
      ) {
        alreadyUsed = true;
      }
  ...
  submitExercise = (bool, e) => {
    ...
    let updatedQuestions = questions.map((question) => {
      return {
        ...
        answer: groups[question.correctGroup.value.split("-").pop() - 1],
      };
    });
    ...
    if (currentQuestion.id <= questions.length) {
      let updatedCurrentQuestion = {
        ...
        answer: groups[currentQuestion.correctGroup.value.split("-").pop() - 1],
      };
    ...
    if (bool)
      this.props.router.navigate("/play/group", {
        state: { exercise: exercise, edit: true },
      });
